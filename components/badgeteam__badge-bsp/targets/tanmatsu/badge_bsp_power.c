...
esp_err_t bsp_power_get_battery_information(bsp_power_battery_information_t* out_information) {
    ...
    ESP_RETURN_ON_ERROR(tanmatsu_coprocessor_get_pmic_charging_status(handle, &battery_attached, &usb_attached,
                                                                      &charging_disabled, &charging_status),
                        TAG, "Failed to get the battery charging status");

    bool    chrg_disabled;
    uint8_t chrg_speed;
    ESP_RETURN_ON_ERROR(tanmatsu_coprocessor_get_pmic_charging_control(handle, &chrg_disabled, &chrg_speed), TAG,
                        "Failed to get charging control status");
...
esp_err_t bsp_power_get_battery_voltage(uint16_t* out_millivolt) {
    ...
    tanmatsu_coprocessor_handle_t handle = NULL;
    ESP_RETURN_ON_ERROR(bsp_tanmatsu_coprocessor_get_handle(&handle), TAG, "Failed to get coprocessor handle");
    ESP_RETURN_ON_ERROR(tanmatsu_coprocessor_get_pmic_vbat(handle, out_millivolt), TAG,
...
esp_err_t bsp_power_get_system_voltage(uint16_t* out_millivolt) {
    ...
    tanmatsu_coprocessor_handle_t handle = NULL;
    ESP_RETURN_ON_ERROR(bsp_tanmatsu_coprocessor_get_handle(&handle), TAG, "Failed to get coprocessor handle");
    ESP_RETURN_ON_ERROR(tanmatsu_coprocessor_get_pmic_vsys(handle, out_millivolt), TAG,
                        "Failed to read system voltage");
    return ESP_OK;
}
...
esp_err_t bsp_power_get_charging_configuration(bool* out_disabled, uint16_t* out_current) {
    ...
    tanmatsu_coprocessor_handle_t handle = NULL;
    ESP_RETURN_ON_ERROR(bsp_tanmatsu_coprocessor_get_handle(&handle), TAG, "Failed to get coprocessor handle");
    ESP_RETURN_ON_ERROR(tanmatsu_coprocessor_get_pmic_charging_control(handle, &disabled, &chrg_speed), TAG,
                        "Failed to get charging configuration");
    if (out_disabled) {
...
esp_err_t bsp_power_configure_charging(bool disable, uint16_t current) {
    ...
    tanmatsu_coprocessor_handle_t handle = NULL;
    ESP_RETURN_ON_ERROR(bsp_tanmatsu_coprocessor_get_handle(&handle), TAG, "Failed to get coprocessor handle");
    ESP_RETURN_ON_ERROR(tanmatsu_coprocessor_set_pmic_charging_control(handle, disable, chrg_speed), TAG,
                        "Failed to configure charging");
    return ESP_OK;
}
...
esp_err_t bsp_power_get_usb_host_boost_enabled(bool* out_enabled) {
    ...
    tanmatsu_coprocessor_handle_t handle = NULL;
    ESP_RETURN_ON_ERROR(bsp_tanmatsu_coprocessor_get_handle(&handle), TAG, "Failed to get coprocessor handle");
    ESP_RETURN_ON_ERROR(tanmatsu_coprocessor_get_pmic_otg_control(handle, out_enabled), TAG,
                        "Failed to get USB host boost status");
    return ESP_OK;
}
...
esp_err_t bsp_power_set_usb_host_boost_enabled(bool enable) {
    tanmatsu_coprocessor_handle_t handle = NULL;
    ESP_RETURN_ON_ERROR(bsp_tanmatsu_coprocessor_get_handle(&handle), TAG, "Failed to get coprocessor handle");
    ESP_RETURN_ON_ERROR(tanmatsu_coprocessor_set_pmic_otg_control(handle, enable), TAG,
                        "Failed to set USB host boost configuration");
    return ESP_OK;
}
...
esp_err_t bsp_power_get_radio_state(bsp_radio_state_t* out_state) {
    ...
    switch (status) {
        ...
        default:
            *out_state = BSP_POWER_RADIO_STATE_OFF;
            break;
        case tanmatsu_coprocessor_radio_state_enabled_bootloader:
            *out_state = BSP_POWER_RADIO_STATE_BOOTLOADER;
            break;
        case tanmatsu_coprocessor_radio_state_enabled_application:
...
esp_err_t bsp_power_set_radio_state(bsp_radio_state_t state) {
    ...
    switch (state) {
        ...
        default:
            target_state = tanmatsu_coprocessor_radio_state_disabled;
            break;
        case BSP_POWER_RADIO_STATE_BOOTLOADER:
            target_state = tanmatsu_coprocessor_radio_state_enabled_bootloader;
            break;
        case BSP_POWER_RADIO_STATE_APPLICATION:
            target_state = tanmatsu_coprocessor_radio_state_enabled_application;
            break;
    }
    tanmatsu_coprocessor_handle_t handle = NULL;
    ESP_RETURN_ON_ERROR(bsp_tanmatsu_coprocessor_get_handle(&handle), TAG, "Failed to get coprocessor handle");
